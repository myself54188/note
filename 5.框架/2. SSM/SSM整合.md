...
1. **用户表（Users）**：
   （用户ID，用户名，密码，电子邮件，手机号，注册日期，最后登录日期）

2. **商品表（Products）**：

   （商品ID，商品名称，商品描述，价格，库存数量，商品分类ID，商品图片URL，上架日期，商品状态（促销））
...
4. **订单表（Orders）**：
   ...
   （订单ID，用户ID，商品ID，订单总金额，送达地址，收件人姓名，收件人电话，订单状态，下单时间，支付时间，发货时间，收货时间）

5. **支付信息表（Payments）**：

   （支付ID，订单ID，用户ID，支付金额，支付方式，支付状态，支付时间）

6. **评价表（Reviews）**：

   （评价ID，商品ID，用户ID，评分，评论内容，评论时间）
